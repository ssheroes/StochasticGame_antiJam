classdef Com < handle
    ...
    methods
        function obj = Com(decay,expl,gamma)
            Player = minimaxAntiJam(decay,expl,gamma);           
        end
        ...
        function ActionSet = findAvaliableAction(obj,state,ChannelNum)   %constructor
            ...
            if PuState ==1
                ActionSet.action = [0,0,0,0];
                ActionSet.Index = obj.actionToIndex(ActionSet.action,ChannelNum);
            else
        ...
        function action =chooseAction(obj,state,ActionSetCom,ActionSetAttack,flagNew)   %Output the action
            action = obj.Player.chooseAction(state,ActionSetCom,ActionSetAttack,flagNew);
