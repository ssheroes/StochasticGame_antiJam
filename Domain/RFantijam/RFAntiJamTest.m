classdef RFAntiJamTest< handle
    ...
    methods
        function obj = RFAntiJamTest(Channel,Pu)
            obj.Channel = Channel;
            obj.Pu = Pu;
        ...
        function testResult = test(obj,Com,Attacker,TestCnt)
           Com.Player.learning = 0;
           Attacker.Player.learning = 0;
           step = 0; 
           obj.restart([0,1,0,0]); 
           testResult = zeros(1,TestCnt+1);
           JamMax = Attacker.JamMax;
           disp('开始测试');
           while step <= TestCnt
               if mod(step,TestCnt/20)==0
                   disp('------------------------------------');
                   fprintf('%4f%%\n',step*100/TestCnt);
